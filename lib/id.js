...
/** Generate enquiry ID — ENQ-YYYY-YY-DD-MM-NNNN. */
export function nextEnquiryId() {
  const n   = nextSeq('counter.enquiry');
  const now = new Date();
  const dd  = String(now.getDate()).padStart(2, '0');
  const mm  = String(now.getMonth() + 1).padStart(2, '0');
  return `ENQ-${currentFY()}-${dd}-${mm}-${String(n).padStart(4, '0')}`;
}

/** Generate quotation ID — QTN-YYYY-YY-DD-MM-NNNN. */
export function nextQuotationId() {
  const n   = nextSeq('counter.quotation');
  const now = new Date();
  const dd  = String(now.getDate()).padStart(2, '0');
  const mm  = String(now.getMonth() + 1).padStart(2, '0');
  return `QTN-${currentFY()}-${dd}-${mm}-${String(n).padStart(4, '0')}`;
}
